...
export async function getAllBlogPosts() {
    const snapshot = await getDocs(collection(db, "projectBlog2025"));
...
export async function addComment(postId, user, text, parentId = null) {
    ...
    return await addDoc(collection(db, "posts", postId, "comments"), {
        authorId: user.uid,
        // Use email instead of displayName
...
// Deletes all deleted comments that have no children (i.e. are orphaned)
export async function cleanupDeletedComments(postId) {
    ...
    // Delete orphaned deleted comments
